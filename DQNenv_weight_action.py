...
class Net(nn.Module):
    def __init__(self, n_states, n_actions, n_hidden):
        ...
        self.hid = nn.Linear(n_hidden, n_hidden)
...
class DQN(object):
    def __init__(self, n_states, n_actions, n_hidden, batch_size, lr, epsilon, gamma, target_replace_iter,
        ...
        ''' NSMQV_toEdge '''
        self.memory = np.zeros((memory_capacity, n_states * 2 + 2))  # 根據需要存的量去調整 size
        ''' NSMQV_Simulation '''
    ...
    def choose_action(self, state):
        ...
        if np.random.uniform() < self.epsilon:  # random
            ...
            ''' NSMQV_toEdge '''
            action = np.random.randint(1, self.n_actions, size=3)
            ''' NSMQV_Simulation '''
            # action = np.random.randint(self.n_actions / 3, size=3)
        else:  # greedy
            actions_value = torch.reshape(self.eval_net(x), (-1,))  # feed into eval net, get scores for each action
            ...
            action = abs(actions_value).data.numpy()
            ''' NSMQV_Simulation '''
    ...
    def learn(self):
        ...
        b_action = torch.LongTensor(b_memory[:, self.n_states:self.n_states + 1])
...
if __name__ == "__main__":
    ...
    for i_episode in range(n_episodes):
        ...
        state = np.array(alloc_UE, dtype=float)  # reset environment to initial state for each episode
        action = np.array(alloc_UE, dtype=float)
        # state = alloc_UE  # reset environment to initial state for each episode
        # action = alloc_UE
        old_reward = 1
        while True:
            # new_reward: QoS違反機率
            new_reward = env.output_QV(action)
            ...
            for i in range(len(action_type)):
                action[i] = 40000000 * (action_type[i]/np.sum(action_type))
            ...
            # Keep the experience in memory
            a = np.argmax(action_type)
            dqn.store_transition(state, a, 1 - new_reward, action)
