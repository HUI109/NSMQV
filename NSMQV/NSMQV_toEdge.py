...
class NSMQV_Simulation(object):
    def __init__(self):
        self.queue_number = 1
        self.theta = [0.01/3, 1/3, 0.05/3]  # 約束時間
        ...
        # self.w = [0, 0, 0]  # urllc, embb, mmtc packet Edge work load (cycles)
        # self.o = [0, 0, 0]  # urllc, embb, mmtc packet Edge2UE output size ((bits)
        # 原資源數
        ...
        self.step_change = self.mu_UE * 0.1

    def resource_allocate(self, mu_UE):
    ...
    def initial_packet(self):
        ...
        for i in range(self.number_packet):
            ...
            service_time = g.getRandomNumber_Exponential(r_task_size) / self.mu_UE_j[
            ...
            self.packet.append(Packet(i + 1, service_type_gen, r_task_size, 0, 0,
    ...
    def summary(self):
        ...
        total_service_time_UE = [0]*self.service_number
        ...
        for i in range(self.number_packet):
            ...
            total_service_time_UE[self.packet[i].service_type] += self.packet[i].service_time  # 0代表queue0= > UE
            ...
            for k in range(self.queue_number):
                total_service_time[k] += self.packet[i].service_time  # 計算某一queue的服務時間
    ...
    def output_QV(self, alloc_change):
        self.resource_allocate(alloc_change)
    ...
    def step(self, action_type):
        ...
        ''' DQNenv '''
        state = self.action_types(action_type, self.step_change * self.qos_violation)
        mu_all = np.array(self.mu_UE_j, dtype=float)
        ''' DQNenv_binary_action '''
        # state = [0, 0, 0]
        # state[action_type[0]] += self.step_change * self.qos_violation
        # state[action_type[1]] -= self.step_change * self.qos_violation
        ...
        # return np.array(state), self.qos_violation, done
...
#     env.resource_allocate(alloc_UE)
#
#     # 初始化封包
#     env.initial_packet()
#
