...
class Actor(object):
    ...
    def choose_action(self, s):
        ...
        return np.argmax(self.sess.run(self.a, feed_dict={S: s}))
...
class Memory(object):
    ...
    def sample(self, n):
        ...
        if self.pointer < BATCH_SIZE:
            indices = np.random.choice(self.pointer, size=n)
        # elif self.pointer < self.capacity & self.pointer > BATCH_SIZE:
        #     indices = np.random.choice(self.pointer, size=n)
        else:
...
M = Memory(MEMORY_CAPACITY, dims=2 * state_dim + 2)
...
for i in range(MAX_EPISODES):
    ...
    s = alloc_UE  # + alloc_E + alloc_EU  # reset environment to initial state for each episode
    ...
    for j in range(MAX_EP_STEPS):
        ...
        a = actor.choose_action(np.array(s))
        ...
        r = 1 - r
        M.store_transition(s_store, a, r / 10, s_)
        ...
        ep_reward += r
        print('QoS violation: ', 1 - r)
        print('State: ', s_store)

        f = open('record.txt', 'a')
        f.write('Episode: {} , Steps: {} , Rewards: {} , QoS violation: {} , Action type: {} \n'.format(i, j, ep_reward, 1 - r, a))
        f.close()

        print('Episode:', i, 'Steps:', j, 'Reward:', ep_reward, 'Explore: %.2f' % var)
        if j == MAX_EP_STEPS-1:
