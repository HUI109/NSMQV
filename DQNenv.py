...
class Net(nn.Module):
    def __init__(self, n_states, n_actions, n_hidden):
        ...
        self.hid1 = nn.Linear(n_hidden, n_hidden)
...
class DQN(object):
    ...
    def learn(self):
        ...
        ''' NSMQV_Simulation '''
        b_state = torch.FloatTensor(b_memory[:, :self.n_states])
        b_action = torch.LongTensor(b_memory[:, self.n_states:self.n_states + 3].astype(int))
        b_reward = torch.FloatTensor(b_memory[:, self.n_states + 3:self.n_states + 4])
        b_next_state = torch.FloatTensor(b_memory[:, -self.n_states:])
